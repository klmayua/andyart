  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl font-bold text-[#F5EBDD]">Corporate Projects</h1>
          <p className="text-sm text-[#7B6854] mt-1">{filtered.length} projects · Est. value: ${(totalValue / 1000).toFixed(0)}k</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filtered.map((p) => (
          <div key={p.id} className="bg-[linear-gradient(180deg,rgba(28,23,20,0.94)_0%,rgba(22,18,16,0.98)_100%)] rounded-[26px] border border-[rgba(214,170,92,0.08)] p-5 hover:border-[rgba(214,170,92,0.12)] transition-all shadow-[0_2px_10px_rgba(0,0,0,0.16)]">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-4 flex-1">
                <div className="w-12 h-12 rounded-xl bg-[rgba(255,255,255,0.05)] flex items-center justify-center flex-shrink-0">
                  <Building2 size={20} className="text-[#C89B4F]" />
                </div>
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <h3 className="font-semibold text-[#F5EBDD]">{p.companyName}</h3>
                    <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STATUS_COLORS[p.status]}`}>{p.status.replace('_', ' ')}</span>
                    {p.priority !== 'standard' && <span className={`text-xs font-bold ${p.priority === 'executive' ? 'text-[#C89B4F]' : 'text-purple-400'}`}>{p.priority.toUpperCase()}</span>}
                  </div>
                  <p className="text-sm text-[#7B6854]/70">{TYPE_LABELS[p.projectType]} · {p.location}</p>
                  <p className="text-xs text-[#7B6854]/50 mt-1 line-clamp-2">{p.description}</p>
                  <div className="flex items-center gap-4 mt-2">
                    {p.roomCount && <span className="text-xs text-[#7B6854]">{p.roomCount} {p.projectType === 'hotel' ? 'rooms' : 'spaces'}</span>}
                    {p.estimatedWorks && <span className="text-xs text-[#7B6854]">{p.estimatedWorks} works</span>}
                    {p.budgetRange && <span className="text-xs text-[#C89B4F] font-medium">{p.budgetRange}</span>}
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-end gap-2">
                {p.assignedTo && <span className="text-xs text-[#7B6854]/50">→ {p.assignedTo}</span>}
                <span className="text-xs text-[#7B6854]/50">{formatDate(p.createdAt)}</span>
                <button onClick={() => setSelected(p)} className="text-xs px-3 py-1.5 bg-[rgba(255,255,255,0.05)] text-[#7B6854] rounded-full hover:bg-[rgba(255,255,255,0.1)] transition-colors">Details</button>
              </div>
            </div>
          </div>
        ))}
        {filtered.length === 0 && (
          <div className="bg-[linear-gradient(180deg,rgba(28,23,20,0.94)_0%,rgba(22,18,16,0.98)_100%)] rounded-[26px] border border-[rgba(214,170,92,0.08)] py-16 text-center text-[#7B6854]/50 shadow-[0_2px_10px_rgba(0,0,0,0.16)]">No projects</div>
        )}
      </div>
