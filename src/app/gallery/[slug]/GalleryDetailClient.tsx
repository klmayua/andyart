...
import type { ArtworkDetail } from '@/data/artworks';

interface GalleryDetailClientProps {
  artwork: ArtworkDetail;
}

export default function GalleryDetailClient({ artwork }: GalleryDetailClientProps) {
  ...
  const handleWishlistToggle = () => {
    if (inWishlist) {
      ...
      track('artwork_wishlist_remove', { page: 'artwork_detail', slug: artwork.slug });
    } else {
      addToWishlist(artwork.id);
      track('artwork_wishlist_add', { page: 'artwork_detail', slug: artwork.slug, title: artwork.title });
    }
  };
          ...
          <div className="space-y-6">
            ...
            {artwork.inStock ? (
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => { track('artwork_inquire', { page: 'artwork_detail', slug: artwork.slug, title: artwork.title }); openInquiry({ artworkTitle: artwork.title, artworkSlug: artwork.slug, artworkPrice: artwork.price, isPriceOnRequest: artwork.isPriceOnRequest }); }}
                ...
                <button
                  onClick={() => { track('service_call_concierge', { page: 'artwork_detail', channel: 'whatsapp', slug: artwork.slug }); openConcierge({ context: `Artwork inquiry: ${artwork.title}` }); }}
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => { track('artwork_talk_curator', { page: 'artwork_detail', slug: artwork.slug }); openConcierge({ context: `Request similar to ${artwork.title}` }); }}
            ...
            <div className="flex flex-wrap gap-3">
              {artwork.inStock && (
                <button
                  onClick={() => { track('artwork_reserve', { page: 'artwork_detail', slug: artwork.slug, title: artwork.title }); openReserve({ artworkTitle: artwork.title, artworkSlug: artwork.slug, artworkPrice: artwork.price }); }}
              ...
              <button
                onClick={() => { track('artwork_private_viewing', { page: 'artwork_detail', slug: artwork.slug, title: artwork.title }); openPrivateViewing({ contextTitle: artwork.title, contextSlug: artwork.slug, contextType: 'artwork' }); }}
                className="text-sm text-andy-bronze hover:text-andy-gold transition-colors flex items-center gap-1.5"
              >
